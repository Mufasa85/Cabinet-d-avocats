...
INSERT INTO
    specialites (nom, slug, descriptions)
VALUES (
        'Droit des Affaires',
        'droit-affaires',
        'Conseil et contentieux en droit des affaires'
    ),
    (
        'Droit Fiscal',
        'droit-fiscal',
    ...
    (
        'Droit Minier',
        'droit-minier',
        'Permis, contrats et compliance minière'
    ),
    (
        'Droit OHADA',
        'ohada',
        'Actes uniformes et droit commercial harmonisé'
    )
...
FROM users
WHERE
    email = 'avocat@cabinet.avocat'
    AND NOT EXISTS (
        SELECT 1
        FROM avocats a
            JOIN users u ON a.user_id = u.id
        WHERE
            u.email = 'avocat@cabinet.avocat'
    );

INSERT INTO
    avocat_specialites (avocat_id, specialite_id)
SELECT a.id, s.id
FROM
    avocats a
    JOIN users u ON a.user_id = u.id
    JOIN specialites s ON s.slug IN (
        'droit-affaires',
        'droit-fiscal'
    )
WHERE
...
FROM users
WHERE
    email = 'stagiaire@cabinet.avocat'
    AND NOT EXISTS (
        SELECT 1
        FROM stagiaires st
            JOIN users u ON st.user_id = u.id
        WHERE
            u.email = 'stagiaire@cabinet.avocat'
    );
